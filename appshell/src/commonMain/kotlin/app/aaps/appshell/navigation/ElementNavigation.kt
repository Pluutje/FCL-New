 ...
 *
 * Only the five that have somewhere to go do anything; the rest are informational, and sending
 * the user to an unrelated screen would be worse than doing nothing.
 *
 * FCL_AI_ADVISOR_READY and FCL_UPDATE_AVAILABLE (09/09/2026, de gebruiker) both just open the FCL
 * vNext plugin screen - the same `it::class.simpleName` lookup as [NavigationRequest.Plugin]
 * hierboven, want deze module is commonMain en mag geen Android-only `.javaClass` gebruiken. Welk
 * tabblad daarna opent (AI Advisor / Analyzer / Updates) wordt NIET hier bepaald: dat blijft een
 * Android-only stap in ComposeMainActivity.kt vóór de aanroep hiernaartoe (FclUpdateNotificationHelper
 * zit in :plugins:aps' androidMain en is dus niet bereikbaar vanuit commonMain).
 */
fun ElementNavigator.handleNotificationAction(notificationId: NotificationId) {
    when (notificationId) {
        ...
        NotificationId.FCL_UPDATE_AVAILABLE    -> {
            val plugin = activePlugin.getPluginsList()
                .find { it::class.simpleName == "OpenAPSFCLPlugin" } ?: return
            openPlugin(plugin, navController, activePlugin)
        }
