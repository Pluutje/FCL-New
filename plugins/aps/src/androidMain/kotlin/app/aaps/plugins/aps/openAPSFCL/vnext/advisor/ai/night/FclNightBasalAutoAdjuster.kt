...
object FclNightBasalAutoAdjuster {
    ...
    private suspend fun applyInternal(
        ...
        // 09/09/2026: SingleProfile is nu immutable (List<Block>, geen JSONArray meer) —
        // deepClone().apply{...} bestaat niet meer, .copy() is de vervanging.
        val newSingleProfile = current.copy(basal = buildBasalBlocks(avgNewHourly))
    ...
    suspend fun applyPending(
        ...
        val newSingleProfile = current.copy(basal = buildBasalBlocks(newHourly))
    ...
    // 09/09/2026: was buildBasalJsonArray() (org.json.JSONArray, voor het oude mutabele
    // SingleProfile.basal). SingleProfile.basal is nu List<Block> (duration in ms, amount
    // per blok) — 24 blokken van elk 1 uur, zelfde volgorde/betekenis als de oude JSON-array.
    private fun buildBasalBlocks(hourly: Map<Int, Double>): List<Block> =
