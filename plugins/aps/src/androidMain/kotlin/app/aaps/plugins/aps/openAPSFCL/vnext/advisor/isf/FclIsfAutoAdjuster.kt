 ...
 *     niet `.sens` — die laatste naam hoort bij het losstaande, interne
 *     OapsProfileFCL/OapsProfileAutoIsf-datamodel (het oref0-porteringspad),
 *     niet bij AAPS's eigen profielopslag.
 ...
 *  3. 09/09/2026: SingleProfile is intussen immutable geworden — `.isf` is
 *     nu `List<Block>` (geen JSONArray meer) en wordt gezet via `.copy(isf = ...)`
 *     i.p.v. `deepClone().apply { isf = ... }`. Zie buildIsfBlocks() hieronder.
...
object FclIsfAutoAdjuster {
    ...
    private suspend fun evaluateInternal(
        ...
        val newSingleProfile = current.copy(isf = buildIsfBlocks(avgNewHourly))
    ...
    suspend fun applyPending(
        ...
        // .isf hieronder — geverifieerd tegen SingleProfile.kt (profile.zip, 16/08/2026): bevestigd correct.
        val newSingleProfile = current.copy(isf = buildIsfBlocks(newHourly))
    ...
    // Zelfde opbouw als buildBasalBlocks() in FclNightBasalAutoAdjuster.kt; de
    // doelveld-naam (.isf) is geverifieerd, zie kdoc bovenaan dit bestand.
    // 09/09/2026: was buildIsfJsonArray() (org.json.JSONArray, voor het oude mutabele
    // SingleProfile.isf). SingleProfile.isf is nu List<Block> (duration in ms, amount
    // per blok) — 24 blokken van elk 1 uur, zelfde volgorde/betekenis als de oude JSON-array.
    private fun buildIsfBlocks(hourly: Map<Int, Double>): List<Block> =
        (0..23).map { h -> Block(T.hours(1).msecs(), hourly[h] ?: 0.0) }
