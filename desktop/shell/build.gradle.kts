...
compose.desktop {
    application {
        ...
        nativeDistributions {
            ...
            // jpackage demands a plain numeric MAJOR[.MINOR[.PATCH]] version - an MSI rejects
            // anything else. Upstream's Versions.appVersion looks like "3.3.0-dev-b-kmp", where
            // substringBefore('-') leaves a valid "3.3.0". This fork's Versions.appVersion instead
            // follows the FCLvNext scheme ("4.0C-v8-v112", see the kdoc there) - substringBefore('-')
            // on that leaves "4.0C", which jpackage rejects (the "C" is not numeric). So the desktop
            // installer version is derived from Versions.versionCode instead - frozen at 1598 (see
            // its kdoc), so this is stable and always a valid MAJOR.MINOR.PATCH, independent of
            // whatever format Versions.appVersion happens to use.
            packageVersion = "1.0.${Versions.versionCode}"
