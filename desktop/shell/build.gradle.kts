...
compose.desktop {
    application {
        ...
        nativeDistributions {
            ...
            // jpackage demands a plain numeric version - an MSI insists on exactly three parts
            // (MAJOR.MINOR.BUILD), a DMG accepts one to three. Versions.appVersion is not that
            // shape: it can carry letters and suffixes (fork example: "4.0B-FCL-v7.6"; dev's own
            // style: "4.0.0-dev-c"). Pull out only the leading digit groups and pad up to three
            // with zeros, so this keeps working no matter what the rest of the version string
            // looks like.
            packageVersion = Regex("""^\d+(\.\d+){0,2}""").find(Versions.appVersion)?.value
                ?.split('.')
                ?.let { parts -> (parts + List(3 - parts.size) { "0" }).joinToString(".") }
                ?: "1.0.0"
