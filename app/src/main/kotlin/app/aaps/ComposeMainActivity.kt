...
class ComposeMainActivity : MetroAppCompatActivity() {
    ...
    private fun handleNotificationAction(notificationId: NotificationId, navController: NavController) {
        // 08-09/09/2026 (de gebruiker): tik op de FCLvNext update-melding → de navigatievlag zetten
        // vóórdat het scherm opent, zodat FCLComposeContent.kt 'm bij het opbouwen van de compositie
        // leest om meteen op het Settings-tabblad met opengeklapte Updates-sectie te starten i.p.v.
        // het dashboard. Moet hier (Android-only) gebeuren: FclUpdateNotificationHelper zit in
        // :plugins:aps' androidMain en is niet bereikbaar vanuit ElementNavigator (commonMain, zie
        // ElementNavigation.kt). Het openen van de FCL-plugin zelf is wél gedeeld (net als
        // FCL_AI_ADVISOR_READY hierna) en gebeurt in handleNotificationAction() daar.
        if (notificationId == NotificationId.FCL_UPDATE_AVAILABLE) FclUpdateNotificationHelper.requestNavigate()
